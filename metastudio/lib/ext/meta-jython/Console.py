...
class Console:
	...
	def handleLine(self, text, noNewInp=0):
		...
		pt = PythonThread(code, self, noNewInp)
...
class PythonThread(Thread):
	def __init__(self, code, console, noNewInp=0):
		self.code = code
		self.console = console
		self.locals = console.locals
		self.noNewInp = noNewInp
...
%(copyright)s
Configured for %(metaversion)s by V. Ganesh
[ To see a list of functions specific to MeTA Studio, type dir(metastudio) ]
[ To use these function in Jython script running in MeTA Studio, use: from metastudio import * ]
""" % {'version':sys.version, 'platform':sys.platform, 'copyright':sys.copyright, 'metaversion':StringResource.getInstance().getVersion()}
...
ideInstance = MainMenuEventHandlers.getInstance(None).getIdeInstance()
jythonFrame = JInternalFrame("Jython Console configured for: " + StringResource.getInstance().getVersion(), 1, 1, 1, 1)
jythonFrame.setLayout(BorderLayout())
jythonFrame.add(pane, BorderLayout.CENTER)
jythonFrame.setVisible(1)
ideInstance.getWorkspaceDesktop().addInternalFrame(jythonFrame)
c.write(header, 'output')
c.handleLine('import metastudio, os', 1)
c.handleLine('from metastudio import *', 1)
