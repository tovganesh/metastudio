...
public class KeywordPatch extends MoveableObjectKeywordParser {
    ...
    public void parse(Tokenizer st) throws Exception {
        ...
        if (type.equals("bilinear"))
            parser.addBilinearPatch(parameters);
        else if (type.equals("bicubic"))
            parser.addBicubicPatch(parameters);
        else
            throw new IllegalArgumentException("Unknown patch type: " + type);
    }
